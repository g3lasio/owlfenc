...
- **Streaming Architecture**: **Simplified single-layer design** - HTTP endpoints (`/process`, `/stream`, `/process-with-files`) return JSON directly for reliability. **Eliminated double-layer SSE pattern** that caused message truncation (Nov 2025).
- **Security Architecture (Nov 2025)**: **Enterprise-grade authentication enforcement** - ALL HTTP agent endpoints (`/process`, `/stream`, `/process-with-files`) require mandatory Firebase token validation. userId is derived EXCLUSIVELY from verified `decodedToken.uid`, never trusted from request body. This prevents user impersonation and ensures secure tool execution, confirmations, telemetry, and audit trails.
- **Transport Layer (Nov 2025)**: **HTTP-primary architecture** - HybridAgentClient uses proven HTTP POST as primary transport (reliable, complete responses). WebSocket deprecated due to persistent truncation issues with callback race conditions and abnormal closures (code 1006). Future roadmap includes Server-Sent Events (SSE) via fetch + ReadableStream for progressive streaming without WebSocket complexity.
