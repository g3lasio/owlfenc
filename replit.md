...
- **WorkflowEngine - Multi-Step Process Automation**: Revolutionary workflow execution system enabling Mervin AI to replicate complete UI workflows through conversational chat. **Architecture**: Declarative TypeScript workflow definitions (`WorkflowDefinition` type with steps, branches, conditions) orchestrated by `WorkflowEngine` core. **State Management**: Dual persistence - Redis for real-time session state (fast reads/writes), PostgreSQL audit log for compliance and debugging. **Step Execution**: Pluggable adapter pattern with `SystemAPIStepAdapter` (calls existing endpoints via SystemAPIService), `DeepSearchStepAdapter` (materials/labor calculations), and `CollectStepAdapter` (gather user input mid-workflow). **Integration**: ChatGPT detects workflow intent (`isWorkflow: true`, `workflowType: 'estimate_wizard'`) → MervinOrchestrator delegates to WorkflowEngine → Engine executes declarative steps → Progress streams via SSE. **Capabilities**: Multi-step data collection with conversational prompts, conditional branching based on context, automatic retries with exponential backoff, resume workflows after interruptions, real-time progress updates, audit trail with step-level granularity. **Implemented Workflows**: EstimateWorkflow (14-step process: collect client → find/create client → collect project details → run DeepSearch → calculate totals → generate PDF → share URL → send email). **Design Philosophy**: Zero code duplication - reuses all existing endpoints, stateful execution enabling human-in-the-loop interactions, declarative definitions making new workflows easy to add. Files: `server/mervin-v2/workflows/` with WorkflowEngine.ts, types.ts, WorkflowSessionRepository.ts, adapters/, and definitions/.
