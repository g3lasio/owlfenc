...
- PROJECTS PAYMENT CURRENCY FIX (2025-10-28): Corregido formatCurrency en ProjectDetails.tsx eliminando división incorrecta por 100 que causaba que $47,467 se mostrara como $474. Cambiado de MXN a USD para consistencia con resto de la app. Los precios ya están almacenados en dólares, no centavos. Architect review: "formatCurrency now formats whole-dollar values correctly in USD".
- PROJECTS CRUD OPERATIONS FIX CRITICAL (2025-10-28): Refactorizado completamente updateProject() en firebase.ts para consistencia con getProjectById(). Root cause: updateProject solo buscaba en colección "projects" causando fallo en guardado de notas, documentos y timeline. Solución: (1) Búsqueda dual en ambas colecciones "projects" y "estimates", (2) Autenticación robusta esperando waitForAuth(), (3) Verificación de propiedad con firebaseUserId/userId antes de writes, (4) Logging detallado para debugging. Impacto: Notas, FileManager (documentos), y FuturisticTimeline (timeline) ahora funcionan correctamente. Architect review: "updateProject locates records in both collections while enforcing authenticated ownership, enabling notes/FileManager to succeed".
