...
- **Conversation History System**: Production-grade conversation persistence and management for Mervin AI. Features authenticated CRUD API (`/api/conversations`) with Firebase token verification, auto-save every 5 messages, automatic title generation using ChatGPT-4o, and intelligent AI model preservation (ChatGPT-4o vs Claude Sonnet 4). Architecture: Firebase Firestore backend (`conversations/{userId}/chats/{conversationId}`), server-side service (`conversation-service.ts`), frontend hook (`useConversationManager`), and sidebar component (`ConversationHistory`) with search/filter capabilities. Security: All endpoints protected with `requireAuth` middleware, userId derived from verified Firebase tokens (never client-supplied), normalized timestamp serialization (ISO strings in API, Date objects in UI). Supports pinning, categorization, export, and deletion. Plan limits: Free Trial (5 conversations), Pro/Master (unlimited, 6-month retention). Model mapping: UI labels ('ChatGPT-4o', 'Claude Sonnet 4') mapped to storage values ('chatgpt', 'claude') for consistency across save/restore flows.
