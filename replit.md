...
- **WorkflowEngine - Multi-Step Process Automation**: Revolutionary workflow execution system enabling Mervin AI to replicate complete UI workflows through conversational chat. **Architecture**: Declarative TypeScript workflow definitions (`WorkflowDefinition` type with steps, branches, conditions) orchestrated by `WorkflowEngine` core. **State Management**: Redis-based session storage with in-memory fallback (PostgreSQL audit log planned for future compliance tracking). **Step Execution**: Pluggable adapter pattern with `SystemAPIStepAdapter` (calls existing endpoints via SystemAPIService with full auth forwarding), and `CollectStepAdapter` (gather user input mid-workflow). **Integration**: ChatGPT detects workflow intent (`isWorkflow: true`, `workflowType: 'estimate_wizard'`) → MervinOrchestrator delegates to WorkflowEngine → Engine executes declarative steps → Progress streams via SSE. **Capabilities**: Multi-step data collection with conversational prompts, conditional branching based on context, automatic retries with exponential backoff, resume workflows after interruptions, real-time progress updates, required context validation before execution. **Implemented Workflows**: EstimateWorkflow (14-step process: collect client → find/create client → collect project details → run DeepSearch → calculate totals → generate PDF → share URL → send email). **Design Philosophy**: Zero code duplication - reuses all existing endpoints via SystemAPIService, stateful execution enabling human-in-the-loop interactions, declarative definitions making new workflows easy to add, required context validation prevents runtime failures. Files: `server/mervin-v2/workflows/` with WorkflowEngine.ts, types.ts, WorkflowSessionRepository.ts, adapters/SystemAPIStepAdapter.ts, and definitions/EstimateWorkflow.ts.
- **File Attachment System**: Multi-format file processing for Mervin AI conversations. Backend processes: PDFs (full text extraction via pdf-parse), text documents (.txt, .md, code files - full content), JSON/CSV (structured data parsing), images (metadata only - dimensions, format, size). FileProcessorService extracts content and generates contextual summaries for AI analysis. Frontend provides intuitive file attachment UI with visual previews showing file type icons, file size validation (10MB limit per file, 5 files max), and one-click removal. Files sent via FormData to `/api/mervin-v2/process-with-files` endpoint using multer middleware. MervinOrchestrator integrates file context seamlessly into ChatGPT and Claude prompts. **Production Robustness**: 120-second request timeout with proper cleanup to prevent unhandled rejections, comprehensive error handling with stream closure guarantees, intelligent task classification (distinguishes "review document" from "create document"), detailed logging pipeline for debugging, and try-catch blocks in all flow methods (conversation, executable tasks, complex queries). All error paths properly close SSE streams and notify client. **Note**: Image OCR not yet implemented - images only contribute metadata. Future enhancement: integrate Tesseract or cloud vision API for text extraction from images/scanned documents. Current use cases: reviewing permit PDFs, analyzing text documents, understanding building codes from uploaded files.
- **Mervin V2 UI/UX Enhancement System**: Revolutionary minimalist interface with real-time transparency components. **ThinkingIndicator** displays dynamic status updates (Investigando, Analizando, Procesando, Generando respuesta) with contextual icons that change based on streaming updates. **MessageContent** component implements ChatGPT-style typing effect (15ms/char) for natural response display with intelligent URL detection and conversion to clickable links (auto-detects https:// URLs, renders with ExternalLink icon, opens in new tab). **SmartContextPanel** shows active endpoints and AI model in use (ChatGPT-4o vs Claude Sonnet 4). **AgentCapabilitiesBadge** differentiates Agent Mode (full autonomous capabilities) from Legacy Mode (basic conversations). **DynamicActionSuggestions** replaces hardcoded menus with contextual action chips that adapt based on conversation flow (estimate, contract, permit, property contexts). **WebResearchIndicator** notifies when Mervin performs web searches with result count. **SystemStatusBar** displays V2 backend health and version in footer. All context states auto-reset on new tasks to prevent stale data. **Dynamic Response Enhancement**: ClaudeService explicitly instructed to include complete, clickable URLs in completion messages (shareable estimate links, PDF downloads, source citations), enabling rich user experiences directly in chat. Design philosophy: Clean, minimal UI with single dynamic indicator instead of multiple competing status messages, maximum user productivity through clickable actions.
