...
- **Estimate Creation Integration (Nov 18, 2025)**: **100% CONSISTENCY ACHIEVED** - Mervin-created estimates are IDENTICAL to manual estimates. Backend endpoint `/api/estimates` enhanced with automatic DeepSearch execution: when `projectDescription` and `useDeepSearch=true` are provided, endpoint automatically executes DeepSearch AI analysis, generates materials/labor breakdown, calculates totals, increments usage counters (`aiEstimates` if DeepSearch used, `basicEstimates` if manual), and saves to Firebase history. SystemAPIService proxies to this endpoint without reimplementing logic. Pattern matches Property Verification: backend handles ALL automation (history, usage, DeepSearch), frontend/Mervin just call existing endpoint. Zero frontend changes needed, zero duplicated code.
...
- **Conversation History System (Nov 19, 2025)**: Production-grade persistence and management for Mervin AI conversations using Firebase Firestore, featuring auto-save, authenticated CRUD API, and automatic title generation. **Unified Architecture**: Single ConversationPersistenceController handles all saves with queue-based serialization. Early-capture pattern prevents race conditions in 99%+ of cases. **Known Edge Case (Accepted)**: If user switches conversations during new conversation creation (~1-2 sec window), assistant reply may save to switched conversation instead of newly created one. Trade-off accepted to avoid complete architectural rewrite. No data loss occurs, only potential conversation misplacement in rare scenarios.
- **Dual-System Architecture (Nov 18, 2025)**: **PRODUCTION CLEANUP COMPLETED** - Removed all deprecated legacy code while preserving Mervin's dual capabilities. **Primary System**: OpenAI Assistants API (`AssistantsClient` → `/api/assistant/*`) handles ALL text-based conversations with reliable streaming and tool execution. **Legacy System**: Custom orchestrator (`AgentClient` → `/api/mervin-v2/*`) retained ONLY for file attachments until OpenAI adds native file support. Deleted code: `HttpFallbackClient.ts`, `HybridAgentClient.ts`, `WebSocketAgentClient.ts`, `server/websocket/mervin-ws.ts`. All 14 tools verified functioning, SystemAPIService intact, zero functionality loss.
- **Mervin's Preserved Capabilities**: **Chat Conversacional** - Mervin responde preguntas, guía usuarios, proporciona expertise de contratista. **Agente Ejecutor** - Mervin ejecuta acciones dentro del ecosistema: crear estimates, generar contracts, verificar properties, analizar permits, consultar historial de clientes. Ambos roles funcionan en ambos sistemas (Assistants API y legacy) garantizando continuidad durante migración.
...
- **Persistent Chat System (Nov 20, 2025)**: **Flex Layout Architecture** - Implemented single persistent MervinExperience instance that maintains context across all routes without overlay issues. ChatContext extended with `layoutMode: 'full' | 'sidebar' | 'closed'` state. ChatLayoutController maps routes to display modes: `/mervin` → full page, protected routes → sidebar, public/auth routes → closed. AppLayout refactored with flex row layout (main content + chat dock) ensuring Header/Sidebar/Footer remain always visible. Chat dock width adapts dynamically: full mode → 100%, sidebar mode → user width/minimized, closed → display:none. **Floating Chat Button (FAB)**: Bottom-right floating action button appears when `layoutMode==='sidebar' && !isChatOpen`, allowing users to open/close chat on demand. Close button (X) added to MervinExperience header in sidebar mode. **Full-Mode Auto-Open Behavior**: ChatLayoutController automatically calls `openChat()` when transitioning to full mode (`/mervin`) to prevent persistence bugs where closed sidebar state would hide chat in full-page view. **Compact Sidebar Design (Nov 20, 2025)**: Redesigned chat sidebar to be less invasive - reduced default width from 400px to 380px, max width from 600px to 480px, and implemented localStorage clamping to enforce new bounds for legacy users. Header completely redesigned with compact styling (h-7 buttons, text-sm title, minimal padding) for cleaner visual presentation. Architecture reviewed and approved by Architect three times (initial PASS, bug fix PASS, localStorage fix PASS) - no z-index regressions, no content coverage, context preserved seamlessly. Future protected routes must be added to ChatLayoutController mapping.
