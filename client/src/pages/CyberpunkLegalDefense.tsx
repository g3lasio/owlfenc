...
export default function CyberpunkLegalDefense() {
  ...
  // Generate and download professional PDF contract
  const generateContractPDF = async () => {
    if (!extractedData) {
      toast({
        title: "Data Missing",
        description: "Contract data is required to generate PDF",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Creating high-quality contract document with professional formatting...",
      });
      ...
      const contractData = {
        client: {
          ...
          address: extractedData.clientInfo?.address || 'Client Address',
          phone: extractedData.clientInfo?.phone || 'Client Phone',
          email: extractedData.clientInfo?.email || 'Client Email'
        },
        contractor: {
          name: profile?.companyName || 'Owl Fenc LLC',
          address: profile?.address || '2901 Owens Court, Fairfield, CA 94534',
          phone: profile?.phone || '202 549 3519',
          email: profile?.email || 'info@owlfenc.com',
          license: profile?.licenseNumber || 'CA-LICENSE-123456'
        },
        project: {
          type: extractedData.projectDetails?.type || 'Construction Project',
          description: extractedData.projectDetails?.description || 'Professional construction services',
        ...
        protectionClauses: intelligentClauses
      ...
      console.log('📄 Sending contract data for PDF generation:', contractData);

      // Generate PDF using the new high-quality service
      ...
      toast({
        ...
        description: `Professional contract downloaded: ${filename}`,
      });
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : 'Unable to generate PDF contract',
        variant: "destructive"
      });
    } finally {
  ...
  const sendContractForSignature = async () => {
    toast({
      ...
      description: "Digital signature functionality will be implemented next",
    });
  };

  // Start new contract workflow
  const startNewContract = () => {
    ...
    toast({
      title: "🔄 NEW CONTRACT INITIATED",
      description: "Ready to create a new legal defense contract",
    });
  };
