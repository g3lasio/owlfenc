...
const waitForAuth = (): Promise<any> => {
  return new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      if (!resolved) {
        console.warn("🔐 [WAIT-AUTH-CLIENT] Timeout - no user found after 5 seconds");
        resolved = true;
    ...
    unsubscribe = onAuthStateChanged(auth, (user) => {
      ...
      if (!resolved && user) {
        ...
        console.log("🔐 [WAIT-AUTH-CLIENT] Transient null user - continuing to wait for session restore...");
      }
    });
  });
};
