...
export async function generateSimpleEstimatePDF(req: Request, res: Response) {
  ...
  try {
    ...
    // Construir datos válidos para PDF Monkey
    const estimateData: EstimateData = {
      ...
      client: {
        name: estimateDataFromFrontend.client?.name || estimateDataFromFrontend.clientName || 'Cliente Sin Nombre',
        email: estimateDataFromFrontend.client?.email || estimateDataFromFrontend.clientEmail || '',
        address: estimateDataFromFrontend.client?.address || estimateDataFromFrontend.clientAddress || '',
        phone: estimateDataFromFrontend.client?.phone || estimateDataFromFrontend.clientPhone || ''
      },
      contractor: {
        companyName: estimateDataFromFrontend.contractor?.companyName || estimateDataFromFrontend.companyName || 'Owl Fence',
        name: estimateDataFromFrontend.contractor?.name || estimateDataFromFrontend.companyName || 'Owl Fence',
        email: estimateDataFromFrontend.contractor?.email || estimateDataFromFrontend.companyEmail || 'info@owlfenc.com',
        phone: estimateDataFromFrontend.contractor?.phone || estimateDataFromFrontend.companyPhone || '202-549-3519',
        address: estimateDataFromFrontend.contractor?.address || estimateDataFromFrontend.companyAddress || '2901 Owens Court',
      ...
      project: {
        ...
        description: estimateDataFromFrontend.project?.description || estimateDataFromFrontend.projectDetails || 'Fence construction project',
        location: estimateDataFromFrontend.project?.location || estimateDataFromFrontend.client?.address || estimateDataFromFrontend.clientAddress || '',
        scopeOfWork: estimateDataFromFrontend.project?.scopeOfWork || estimateDataFromFrontend.notes || 'Fence installation according to specifications'
      },
      items: (estimateDataFromFrontend.items || []).map((item: any) => ({
      ...
      subtotal: Number(estimateDataFromFrontend.subtotal) || 0,
      tax: Number(estimateDataFromFrontend.tax) || Number(estimateDataFromFrontend.taxAmount) || 0,
      taxRate: Number(estimateDataFromFrontend.taxRate) || 10,
      total: Number(estimateDataFromFrontend.total) || 0,
      notes: estimateDataFromFrontend.notes || 'Generated by Owl Fence'
    };
