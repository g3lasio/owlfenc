...
/**
 * Crea un enlace de pago utilizando Stripe
 */
async createPaymentLink(options: PaymentLinkOptions): Promise<string> {
  const { amount, description, successUrl, cancelUrl, customerEmail, userId, projectId } = options;
  
  try {
    console.log(`[${new Date().toISOString()}] Creando enlace de pago - Monto: ${amount}, Descripción: ${description}`);
    ...
    // Crear un producto para este pago
    const product = await stripe.products.create({
      name: description,
      metadata: {
        userId: userId.toString(),
        projectId: projectId ? projectId.toString() : 'none'
      }
    });
    
    // Crear un precio único para este pago
    const price = await stripe.prices.create({
      product: product.id,
    ...
    // Guardar el enlace de pago en la base de datos
    const paymentLink: InsertProjectPayment = {
      userId,
      projectId,
      type: 'custom',
      status: 'pending',
    ...
    await storage.createProjectPayment(paymentLink);
    
    console.log(`[${new Date().toISOString()}] Enlace de pago creado - URL: ${session.url.substring(0, 60)}...`);
    return session.url;
  } catch (error) {
    console.error(`[${new Date().toISOString()}] Error al crear enlace de pago:`, error);
    throw error;
  }
}
