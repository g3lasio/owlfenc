...
<body>
  ...
  /**
   * Procesa la plantilla HTML reemplazando los placeholders con datos reales
   * @param templateHtml - Plantilla HTML con placeholders
   * @param estimateData - Datos del estimado para rellenar la plantilla
   * @returns HTML procesado con los datos insertados
   */
  private processTemplateWithData(templateHtml: string, estimateData: any): string {
    try {
      let html = templateHtml;
      
      // Definir función para formatear dimensiones
      const formatDimensions = (dimensions: any): string => {
        if (!dimensions) return 'N/A';
        
        const parts = [];
        if (dimensions.length) parts.push(`Largo: ${dimensions.length}ft`);
        if (dimensions.height) parts.push(`Alto: ${dimensions.height}ft`);
        if (dimensions.width) parts.push(`Ancho: ${dimensions.width}ft`);
        if (dimensions.area) parts.push(`Área: ${dimensions.area}ft²`);
        
        return parts.length > 0 ? parts.join(', ') : 'N/A';
      };
      
      // Función para generar filas de la tabla de costos
      const generateEstimateTableRows = (estimateData: any): string => {
        let rows = '';
        
        // Iterar por los componentes de costo
      ...
      const getEstimateSubtotal = (estimateData: any): number => {
        if (!estimateData.costs || !Array.isArray(estimateData.costs)) {
          return 0;
        }
        
        return estimateData.costs.reduce((sum: number, cost: any) => {
          return sum + (cost.total || 0);
        }, 0);
      };
      
      // Función para obtener impuesto
      const getEstimateTax = (estimateData: any): number => {
        if (!estimateData.costs || !Array.isArray(estimateData.costs)) {
          return 0;
        }
        
        const subtotal = getEstimateSubtotal(estimateData);
        return subtotal * (DEFAULT_TAX_RATE || 0);
      };
      
      // Función para obtener total
      const getEstimateTotal = (estimateData: any): number => {
        if (!estimateData.costs || !Array.isArray(estimateData.costs)) {
          return 0;
        }
        
        const subtotal = getEstimateSubtotal(estimateData);
        const tax = getEstimateTax(estimateData);
        return subtotal + tax;
      };
      
      // Función para obtener tiempo estimado
      const getEstimateCompletionTime = (estimateData: any): string => {
        if (!estimateData.project) {
          return '7-14';
        }
        
        return estimateData.project.completionTime || '7-14';
      };
      ...
      const replacements: Record<string, string> = {
        ...
        '[SUBTOTAL]': '$' + formatCurrency(getEstimateSubtotal(estimateData)),
        '[TAX_RATE]': ((DEFAULT_TAX_RATE || 0) * 100) + '%',
        '[TAX_AMOUNT]': '$' + formatCurrency(getEstimateTax(estimateData)),
        '[TOTAL]': '$' + formatCurrency(getEstimateTotal(estimateData)),
        '[COMPLETION_TIME]': getEstimateCompletionTime(estimateData)
      };
